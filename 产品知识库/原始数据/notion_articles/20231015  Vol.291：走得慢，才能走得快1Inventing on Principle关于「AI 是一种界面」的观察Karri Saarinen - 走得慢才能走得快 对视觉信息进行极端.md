# 2023/10/15  Vol.291：走得慢，才能走得快1Inventing on Principle关于「AI 是一种界面」的观察Karri Saarinen - 走得慢才能走得快 对视觉信息进行极端压缩

原文链接: https://pmthinking.notion.site/Inventing-on-Principle-2b42f8e86e1580c1bb91e278c90fb30a?pvs=25
...
🔆这可能是我见过的最好的演讲了。它不仅对我，而且对整个编程社区都产生了巨大的影响; 从这次演讲中得到的想法最终被广泛使用的开发工具所采用，例如 Chrome 检查器。更广泛地说，它展示了一个计算可能是什么的愿景，维克多仍然在堂吉诃德式地追求，主要是通过 Dynamicland 项目。这种愿景很难描述，但一旦你看到它，你就会开始哀叹我们所有人都用过的工具都是垃圾。
—— James Somers.
...
游戏调试。在实时编程的基础上，可容易的让你看到程序参数对游戏的调整，对游戏过程的可视化调试。

算法调试。在写二分查找算法时可以实时看到程序的执行过程。边写边看到。

电路图。可以实时地看到电路图中各个部件的对1/0信号的处理。

动画。一种比 flash 制作动画更厉害的方法。
...
而 Larry 所做的完全是对特定文化背景的反应。

Larry Tesla 解决了非模态文本操作的问题
...
Inventing on Principle  transcript 英文文本
